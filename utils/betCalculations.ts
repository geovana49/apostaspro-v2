...
export const calculateBetStats = (bet: Bet) => {
    ...
    // Debug logging
    if (bet.coverages.length > 1 && bet.coverages.filter(c => c.status === 'Green').length > 1) {
        console.log('🔍 Multi-Green Bet Calculation:', {
            event: bet.event,
            isFreebetConversion,
            coverages: bet.coverages.map((c, i) => ({
                index: i,
                bookmaker: c.bookmakerId,
                stake: c.stake,
                odd: c.odd,
                import { Bet } from '../types';
